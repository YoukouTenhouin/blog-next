    ...
    return (
        <>
            {lines.map((l, idx) => {
                ...
                return (
                    <Window.Termline key={idx} pixel={currentLang === "zh"}>
                        {l.map((s) => {
                            switch (s.type) {
                                case "image":
                                    return (
...
function kv(en_k: string, en_v: string, zh_k: string, zh_v: string): TermLine;
function kv(en_k: string, v: string, zh_k: string): TermLine;
function kv(
    en_k: string,
    en_v: string,
    zh_k?: string,
    ...
    if (zh_k) {
        return [
            ...
            {
                type: "text",
                content: zh_v ? { en: en_v, zh: zh_v } : en_v,
            },
        ];
    } else {
        return [
...
const printGreeting = (vm: MiniVM) => {
    vm.printLines(
        ...
        .printSection({ type: "text", content: { en: "Type ", zh: "输入 " } })
        .printSection({ type: "text", content: "'help'", commandOnClick: "help" })
        .printSection({
            type: "text",
            content: { en: " for avaliable commands.", zh: " 以查看可用命令列表。" },
        })
        .newLine()
        .printSection({ type: "text", content: { en: "Type ", zh: "输入 " } })
        .printSection({
        ...
        .printSection({
            type: "text",
            content: { en: " to execute autorun demo.", zh: " 以执行全自动展示。" },
        })
        .newLine();
};
                        ...
                        <div className={styles.about_wrapper}>
                            <div
                                className={clsx({
                                    ...
                                    [pixelFont.className]: currentLang == "zh",
                                })}
                            >
                                <TermWindow
                                    vm={vm}
